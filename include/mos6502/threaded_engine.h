...
#define PULL_FROM_STACK(chip) \
    ( (chip)->stack_pointer++, \
      (chip)->memory[0x0100 | (chip)->stack_pointer] )
...
#define AND_VALUE(chip, value)                             \
    do {                                                   \
        ...
        SET_ZERO_AND_NEGATIVE_FLAGS(                       \
            chip, (chip)->accumulator);                    \
    } while (0)
...
#define CMP_VALUE(chip, value)                                 \
    do {                                                       \
        uint8_t a = (chip)->accumulator;                       \
        uint16_t diff = (uint16_t)a - (uint16_t)(value);       \
        SET_ZERO_AND_NEGATIVE_FLAGS(chip, (uint8_t)diff);      \
        SET_CARRY(chip, a >= (value));                         \
    } while (0)
...
#define CPX_VALUE(chip, value)                                 \
    do {                                                       \
        uint8_t x = (chip)->index_x_register;                  \
        uint8_t diff = (uint8_t)(x - (value));                 \
        SET_ZERO_AND_NEGATIVE_FLAGS(chip, diff);               \
        SET_CARRY(chip, x >= (value));                         \
    } while (0)
...
#define CPY_VALUE(chip, value)                                 \
    do {                                                       \
        uint8_t y = (chip)->index_y_register;                  \
        uint8_t diff = (uint8_t)(y - (value));                 \
        SET_ZERO_AND_NEGATIVE_FLAGS(chip, diff);               \
        SET_CARRY(chip, y >= (value));                         \
    } while (0)
...
#define OP_CLC(chip, instruction)       \
    do {                                \
        SET_CARRY(chip, false);         \
        NEXT(chip, instruction);        \
    } while (0)

#define OP_CLD(chip, instruction)       \
    do {                                \
        SET_DECIMAL(chip, false);       \
        NEXT(chip, instruction);        \
    } while (0)

#define OP_CLI(chip, instruction)               \
    do {                                        \
        SET_INTERRUPT_DISABLE(chip, false);     \
        NEXT(chip, instruction);                \
    } while (0)

#define OP_CLV(chip, instruction)       \
    do {                                \
        SET_OVERFLOW(chip, false);      \
        NEXT(chip, instruction);        \
    } while (0)

#define OP_SEC(chip, instruction)       \
    do {                                \
        SET_CARRY(chip, true);          \
        NEXT(chip, instruction);        \
    } while (0)

#define OP_SED(chip, instruction)       \
    do {                                \
        SET_DECIMAL(chip, true);        \
        NEXT(chip, instruction);        \
    } while (0)

#define OP_SEI(chip, instruction)               \
    do {                                        \
